...
func registerTools(server *mcp.Server, client *api.Client, cfg *config.Config) {
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Check which integrations are connected (GitHub, GitLab, Slack, PagerDuty, Datadog, Jira, Linear, Confluence, PostHog, Vercel, Cloudflare, Railway, Fly.io, Nebius, Beam, Daytona, Supabase, PlanetScale, Neon, ClickHouse, ArgoCD, Argo Rollouts, Flux CD, Terraform Cloud).",
	}, func(ctx context.Context, req *mcp.CallToolRequest, args emptyArgs) (*mcp.CallToolResult, any, error) {
